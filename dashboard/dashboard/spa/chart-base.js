...
tr.exportTo('cp', () => {
  ...
  class ChartBase extends Polymer.GestureEventListeners(cp.ElementBase) {
    ...
    isTooltipShowing_(tooltip) {
      return tooltip && tooltip.isVisible && tooltip.rows.length;
    }
    ...
    async onMouseMoveMain_(event) {
      ...
      this.debounce('mousemove-main', async() => {
        ...
        // Ideally, an on-mouseleave listener would dispatchEvent so that
        // callers could hide the tooltip and whatever else when the mouse
        // leaves the main area. However, mouseleave and mouseout are flaky in
        // obnoxious ways. CSS :hover seems to be robust, so a :hover selector
        // sets the property --mouse: inside. mouseLeaveMain_ polls this
        // property.
        const mouseLeave = this.mouseLeaveMain_();

        this.dispatchEvent(new CustomEvent('get-tooltip', {
          bubbles: true,
          composed: true,
          detail: {nearestPoint, nearestLine, mouseLeave},
        }));
